---
title: "Lab1 - Nobel laureates"
author: "Group 6-Lab 1"
output: 
  tufte::tufte_html:
    css: "css/lab.css"
...
Go to the course GitHub organization and locate your lab repo, which should be named `lab1-YourTeamNumber`.
Grab the URL of the repo, and clone it in RStudio.
First, open the R Markdown document `lab1.Rmd` and Knit it.
...
-   Update the YAML, changing the author name to your team number (e.g., "Team 1"), and **knit** the document.
...
nobel <- read.csv("data/nobel.csv") # Or, you can use nobel <- ezpickr::pick("data/nobel.csv")
```
...
```{r}
print("This is the Label for Role # 3")
nrow(nobel)
ncol(nobel)
view(nobel)
```

Ans 1) There are 935 observations and 26 variables in the dataset. Each row in the data set represents a record about nobel prize won by a nobel laureate and the details like category, city, country, etc.

There are some observations in this dataset that we will exclude from our analysis to match the Buzzfeed results.

2.  [Answer is provided below] Create a new data frame called `nobel_living` that filters for
...
Yes, after filtering we get 228 observations in new dataset nobel_living.
```{r}
nrow(nobel_living)
```
...
ggplot(data = nobel_living_science, aes(x = country_us))+ 
  geom_bar(color="darkblue", fill="#69b3a2") + 
  facet_wrap(category ~.) + coord_flip() + 
  labs( x= "Country when the laureate won the prize", y = "Number of Nobel Prizes", title = "Category of Prize v/s Whether the laureate was in US or not")
```

## Interpretation: The Buzzfeed article tells us about the importance of why having foreign representation in American scientific institutes is important. Of the many nobel laureates that were based in the US, many of them were born in a different country, meaning they were immigrants. The visualization clearly indicates that more people were based in US when they won the nobel prize (across all 4 fields) than they were in other countries. A little research tells us that US is the country that is home to 400 nobel laureates today (the most in the world) and even up until the mid 80s around 40% of all living nobel laureates were immigrants who came to the US as a result of oppression or adverse circumstances in their home countries and that number has not changed by a lot even today. Looking at these visualizations we can definitely say that immigration has had a profound impact on the scientific advancements in the States. The data supports it and the Buzzfeed article also makes a point for it. 
...
## Answer: 123 nobel laureates were born outside USA.
...
ggplot(data = nobel_living_science, aes(y = country_us,  fill = born_country_us))+ 
  ...
  labs( x= "Count of Nobel Prizes", y = "Country the laureate was in when he received the prize")
```

🧶 ✅ ⬆️ Knit, *commit, and push your changes to GitHub with an appropriate commit message. Make sure to commit and push all changed files so that your Git pane is cleared up afterwards.*

## Interpretation: In Question 3, across all 4 fields more laureates were "based in USA" (across all 4 fields) than any other country. After introducing a new variable for laureates "born in USA", the interpretation also changes. Now, for all the fields except Economics, more laureates were born outside of USA. This aligns strongly with the points highlighted in the Buzzfeed article that immigration is important for scientific advancements of a country and has helped USA in many ways.
...
## Answer: Germany and United Kingdom are the 2 countries with most laureates who won their prize in US but were born outside of US.
